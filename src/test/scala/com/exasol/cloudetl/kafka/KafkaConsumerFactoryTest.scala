...
class KafkaConsumerFactoryTest extends AnyFunSuite with MockitoSugar {
  ...
  test("apply returns a default kafka consumer type") {
    val consumerProperties = KafkaConsumerPropertiesSupport.create(defaultProperties, mock[ExaMetadata])
    val kafkaConsumer =
      KafkaConsumerFactory.apply(consumerProperties, new VoidDeserializer, new VoidDeserializer)
