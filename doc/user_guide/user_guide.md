...
```sh
sha256sum exasol-kafka-connector-extension-1.7.10.jar
```
...
`target/scala-2.12/exasol-kafka-connector-extension-1.7.10.jar`.
...
```bash
curl -X PUT -T exasol-kafka-connector-extension-1.7.10.jar \
...
CREATE OR REPLACE JAVA SET SCRIPT KAFKA_CONSUMER(...) EMITS (...) AS
  ...
  %jar /buckets/bfsdefault/<BUCKET>/exasol-kafka-connector-extension-1.7.10.jar;
/
