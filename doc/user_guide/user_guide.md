...
```sh
sha256sum exasol-kafka-connector-extension-1.7.13.jar
```
...
`target/scala-2.12/exasol-kafka-connector-extension-1.7.13.jar`.
...
```bash
curl -X PUT -T exasol-kafka-connector-extension-1.7.13.jar \
...
CREATE OR REPLACE JAVA SET SCRIPT KAFKA_CONSUMER(...) EMITS (...) AS
  ...
  %jar /buckets/bfsdefault/<BUCKET>/exasol-kafka-connector-extension-1.7.13.jar;
/
